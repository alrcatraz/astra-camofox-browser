...
| Adaptation | Details |
|:-----------|:--------|
| **VNC watcher** | Display-aware liveness (`-displayfd` socket owner check), zombie-safe; attaches x11vnc to the live browser display and falls back to a 1920×1080 Xvfb while the browser is down; writes the upstream `VNC_STATUS_FILE` contract (`GET /vnc/status`) |
| **Viewport** | Browser viewport hardcoded to 1920×1080 to match the VNC virtual screen (upstream defaults to 1280×720 with a null viewport, causing letterboxing/offset) |
| **Container** | Podman-first Makefile; exec-form `CMD` so SIGTERM reaches node (graceful shutdown in <1s); host-preinstalled `node_modules` copied in (upstream `npm ci` needs a compiler toolchain for better-sqlite3); optional `Dockerfile.astra` thin-layer build on the existing VNC image to skip GFW-hostile apt/npm downloads |
| **Access control** | Requires `CAMOFOX_ACCESS_KEY` on all routes (except `/health`) — upstream's evaluate endpoint has no auth of its own |
