...
$D exec mk-nixci sh -c 'NIXPKGS_ALLOW_INSECURE=1 nix-env -p /nix/var/nix/profiles/ci-cache -f "<nixpkgs>" -iA nodejs_22 python3 gcc gnumake git curl \
...
$D exec mk-nixci sh -c 'for b in node npm npx corepack python3 git curl gcc g++ cc c++ make ld as; do ln -sf /nix/var/nix/profiles/ci-cache/bin/$b /usr/local/bin/$b 2>/dev/null; done; mkdir -p /usr/local/lib; ln -sfn /nix/var/nix/profiles/ci-cache/lib/node_modules /usr/local/lib/node_modules; true'
